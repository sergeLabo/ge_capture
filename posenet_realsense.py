...
class PosenetRealsenseViewer:
    ...
    def viewer(self):
        ...
        if time() - self.t0 > 10:
            print("FPS Posenet =", int(self.nbr/10))
...
class PosenetRealsense(MyRealSense, MyPosenet, PosenetRealsenseViewer):
    ...
    def __init__(self, conn, current_dir, config):
        ...
        self.threshold_p = float(self.config['pose']['threshold'])
        ...
        self.profondeur_maxi = int(self.config['pose']['profondeur_maxi'])
        self.profondeur_mini = int(self.config['pose']['profondeur_mini'])
        self.largeur_maxi = int(self.config['pose']['largeur_maxi'])

        self.histo = [0] * int(self.config['pose']['pile_size'])
        self.lissage = int(self.config['pose']['lissage'])

    def pose_receive_thread(self):
        print("Lancement du thread receive dans posenet")
        t_pose = Thread(target=self.pose_receive)
        t_pose.start()

    def pose_receive(self):
        while self.pose_conn_loop:
            data = self.pose_conn.recv()
    ...
    def main(self, outputs):
        ...
        if outputs:
            ...
            self.skelets_2D = MyPoseNetConversion(outputs, self.threshold_p).skeletons
            ...
            if self.skelets_2D:
                ...
                if self.skelets_2D:
                    ...
                    self.get_who()
                    if self.depth:
                        self.pose_conn.send(['depth', int(self.depth)])

                    zoom = self.get_perso_zoom()
                    if zoom is not None:
                        self.pose_conn.send(['zoom', zoom])

                    self.draw_text()
                    # Dessin
                    self.draw_all_poses()

    def get_perso_zoom(self):
    ...
    def get_who(self):
        ...
        if self.centers:
            ...
            if abs_all_x:  # si abs_all_x is not [], alors all_x est aussi
                ...
                depth = self.centers[self.who][2]
                self.depth = self.get_smooth_depth(depth)
                # La position en x pour affichage
                self.x =  self.centers[self.who][0]
        else:
            self.who, self.depth, self.x = None, None, None

    def get_smooth_depth(self, depth):
        """Lissage de la profondeur"""
